...
try {
    if ($method === 'GET') {
        ...
        if ($search !== '') {
            $sql .= " AND (category LIKE ? OR description LIKE ? OR payee_name LIKE ? OR receipt_number LIKE ?)";
            $like = "%$search%";
            $params[] = $like;
            $params[] = $like;
            $params[] = $like;
    ...
    elseif ($method === 'POST') {
        ...
        // Frontend sends: reference, payee_name, expense_category, description, amount, expense_date, payment_method, notes
        ...
        $category = trim((string)($input['expense_category'] ?? $input['category'] ?? ''));
    ...
    elseif ($method === 'PUT') {
        ...
        $reference = array_key_exists('reference', $input) ? trim((string)$input['reference']) : $existing['reference'];
        $payee_name = array_key_exists('payee_name', $input) ? trim((string)$input['payee_name']) : $existing['payee_name'];
        $category = array_key_exists('expense_category', $input) ? trim((string)$input['expense_category']) : (array_key_exists('category', $input) ? trim((string)$input['category']) : $existing['category']);
