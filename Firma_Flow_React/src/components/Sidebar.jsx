...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed md:relative flex flex-col w-[260px] h-screen max-h-screen text-center shadow-lg rounded-lg bg-white items-start p-2 z-50 transition-transform duration-300 ease-in-out overflow-hidden ${
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-2 hover:bg-slate-100 rounded-lg transition md:hidden z-10"
        >
          <X size={20} className="text-slate-600" />
        </button>

        <div className="flex justify-between items-center w-full mb-4 flex-shrink-0">
          <div className="flex gap-2 items-center justify-start group px-2 py-2 rounded-2xl w-full ease-out-in hover:bg-gradient-to-br from-[#eceef8] to-[#f9f7fa]">
                  ...
                  <p
                    className={`m-0 text-left ml-12 text-normal font-md flex-1 ${
                      isActive
                        ...
                        : "text-slate-600 hover:text-slate-800 hover:font-semibold"
                    }`}
                  >
                    {page.name}
                  </p>
        ...
        <div className="flex-shrink-0 w-full border-t border-slate-300">
          <div className="flex w-full justify-start items-center gap-3 py-3">
            ...
            <div>
              ...
              <p className="font-normal text-slate-500">{userData.company}</p>
            </div>
          </div>

          <button
            onClick={handleLogout}
            className="w-full text-center group/logout py-3 px-4 rounded-lg font-medium text-slate-700 hover:bg-gradient-to-br from-red-50 to-red-100 hover:text-red-600 transition-all duration-200 border border-transparent hover:border-red-200"
          >
            Logout
          </button>
        </div>
