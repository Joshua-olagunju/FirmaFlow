...
import { ChevronDown, User, Menu, TrendingUp, TrendingDown, AlertTriangle } from "lucide-react";
import {
  LineChart,
  Line,
  XAxis,
...
const Dashboard = () => {
  ...
  const salesData = dashboardData?.sales_trend?.map((day) => ({
  ...
  const productData = dashboardData?.top_products?.slice(0, 10).map((product) => ({
  ...
  if (error) {
    return (
      <Layout onMenuClick={(fn) => (openSidebarRef.current = fn)}>
        <div className="flex items-center justify-center h-screen">
          <div className={`${theme.bgCard} p-8 rounded-lg ${theme.shadow} max-w-md text-center`}>
            <AlertTriangle className="mx-auto mb-4 text-red-500" size={48} />
            <h3 className={`text-xl font-bold ${theme.textPrimary} mb-2`}>Error Loading Dashboard</h3>
      ...
      <div className="w-full flex flex-col flex-1 gap-4 md:gap-8 px-2 md:px-4">
        <div className="w-full flex justify-between items-start rounded-b-lg align-top p-4 md:p-6 bg-gradient-to-br from-[#667eea] to-[#764ba2] mt-0">
          ...
          <div className="flex-col items-center pb-2 md:pb-4 flex-1">
            ...
            <p className="m-0 text-xs md:text-base font-500 text-slate-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 w-full">
          ...
          <div className={`flex ${theme.shadow} rounded-lg items-center ${theme.bgCard} p-3 md:p-4`}>
            <div className="flex items-center gap-3 md:gap-4 w-full">
              ...
              <div className="flex flex-col gap-1 min-w-0">
                <p className={`m-0 font-bold ${theme.textPrimary} text-2xl md:text-3xl truncate`}>
                  {dashboardData?.customers?.total_customers || 0}
                </p>
                <p className={`m-0 font-normal ${theme.textSecondary} text-sm md:text-base`}>
                  Total Customers
                </p>
          ...
          <div className={`flex ${theme.shadow} rounded-lg items-center ${theme.bgCard} p-3 md:p-4`}>
            <div className="flex items-center gap-3 md:gap-4 w-full">
              <div className="flex shadow-lg rounded-lg items-center justify-center bg-gradient-to-br from-[#667eea] to-[#764ba2] p-4 md:p-6 flex-shrink-0"></div>
              <div className="flex flex-col gap-1 min-w-0">
                <p className={`m-0 font-bold ${theme.textPrimary} text-xl md:text-2xl truncate`}>
                  {formatCurrency(parseFloat(dashboardData?.sales?.total_sales || 0))}
                </p>
                <p className={`m-0 font-normal ${theme.textSecondary} text-sm md:text-base`}>
                  Total Sales
                </p>
                <p className={`font-normal text-xs ${theme.textSecondary}`}>
                  {formatCurrency(parseFloat(dashboardData?.sales?.today_sales || 0))} today
                </p>
              </div>
            </div>
          </div>
          ...
          <div className={`relative flex ${theme.shadow} rounded-lg items-center ${theme.bgCard} p-3 md:p-4`}>
            <div className="flex items-center gap-3 md:gap-4 w-full">
              <div className="flex shadow-lg rounded-lg items-center justify-center bg-gradient-to-br from-[#667eea] to-[#764ba2] p-4 md:p-6 flex-shrink-0"></div>
              <div className="flex flex-col gap-1 min-w-0">
                <p className={`m-0 font-bold ${theme.textPrimary} text-2xl md:text-3xl truncate`}>
                ...
                <p className={`font-normal text-xs ${theme.textSecondary}`}>
                  {dashboardData?.products?.out_of_stock_products || 0} out of stock
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 w-full">
          ...
          <div className={`${theme.bgCard} relative after:content-[''] after:absolute after:top-0 after:left-0 after:w-full after:h-20 md:after:h-24 after:mb-2 after:${
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center py-2 md:py-4 mb-4 relative z-10 gap-2">
              <h2 className={`text-xl md:text-3xl font-semibold ${theme.textPrimary}`}>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={salesData}>
                    ...
                    <Tooltip 
                      ...
                      contentStyle={{ 
                        backgroundColor: theme.mode === 'light' ? '#fff' : '#1f2937',
          ...
          <div className={`${theme.bgCard} rounded-2xl ${theme.shadow} p-4 md:p-6 relative after:content-[''] after:absolute after:top-0 after:left-0 after:w-full after:h-20 md:after:h-24 after:mb-8 after:${
              theme.mode === "light" ? "bg-slate-100" : "bg-slate-800"
            } after:z-0`}>
            <h2 className={`text-lg md:text-2xl font-semibold ${theme.textPrimary} mb-4 relative z-10`}>
            ...
            {productData.length > 0 ? (
              <div className="flex flex-col items-center">
                ...
                <div className="relative -top-20 md:-top-24 text-center">
                  ...
                  <div className={`text-sm ${theme.textSecondary}`}>
                    {formatCurrency(productData.reduce((sum, item) => sum + item.value, 0))}
                  </div>
                </div>
                ...
                <div className="mt-1 grid grid-cols-1 gap-1 max-h-32 overflow-y-auto w-full px-2">
                  {productData.map((entry, index) => (
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <span className={`text-xs ${theme.textSecondary} ml-auto flex-shrink-0`}>
                        {entry.quantity}
                      </span>
                    </div>
                  ))}
              ...
              <div className="flex items-center justify-center h-48">
                <p className={`${theme.textSecondary} text-center`}>No product sales data</p>
              </div>
            )}
          </div>
        </div>
        {/* Quick Actions - Mobile Responsive */}
        <div className={`${theme.bgCard} rounded-2xl ${theme.shadow} p-4 md:p-6 relative after:content-[''] after:absolute after:top-0 after:left-0 after:w-full after:h-12 md:after:h-14 after:mb-8 after:${
            theme.mode === "light" ? "bg-slate-100" : "bg-slate-800"
          } after:z-0 w-full`}>
          <h2 className={`relative z-10 font-semibold text-xl md:text-3xl ${theme.textPrimary} mb-4`}>
            Quick Actions
          </h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 w-full">
          ...
          <div className={`${theme.bgCard} rounded-2xl ${theme.shadow} p-4 md:p-6 relative after:content-[''] after:absolute after:top-0 after:left-0 after:w-full after:h-16 md:after:h-18 after:mb-8 after:${
              theme.mode === "light" ? "bg-slate-100" : "bg-slate-800"
            } after:z-0 lg:col-span-2`}>
            <div className="flex w-full items-center justify-between relative z-10 mb-4">
              <h2 className={`font-semibold text-xl md:text-3xl ${theme.textPrimary}`}>
                Recent Sales
              </h2>
            ...
            {dashboardData?.recent_sales && dashboardData.recent_sales.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[500px]">
                  <thead>
                    <tr className={`border-b ${theme.mode === 'light' ? 'border-gray-200' : 'border-gray-700'}`}>
                      <th className={`text-left py-2 px-2 text-xs md:text-sm ${theme.textSecondary}`}>Date</th>
                      <th className={`text-left py-2 px-2 text-xs md:text-sm ${theme.textSecondary}`}>Customer</th>
                      <th className={`text-right py-2 px-2 text-xs md:text-sm ${theme.textSecondary}`}>Amount</th>
                      <th className={`text-center py-2 px-2 text-xs md:text-sm ${theme.textSecondary}`}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dashboardData.recent_sales.slice(0, 8).map((sale, index) => (
                      <tr key={index} className={`border-b ${theme.mode === 'light' ? 'border-gray-100' : 'border-gray-800'}`}>
                        <td className={`py-2 px-2 text-xs md:text-sm ${theme.textPrimary}`}>
                          {new Date(sale.date).toLocaleDateString()}
                        </td>
                        <td className={`py-2 px-2 text-xs md:text-sm ${theme.textPrimary} truncate max-w-[150px]`}>
                          {sale.customer_name || 'Walk-in Customer'}
                        </td>
                        <td className={`py-2 px-2 text-xs md:text-sm ${theme.textPrimary} text-right font-semibold`}>
              ...
              <div className="flex flex-col items-center justify-center gap-2 py-8 md:py-16">
                <p className={`${theme.textSecondary} font-semibold text-base md:text-xl`}>
                  No recent sales
                </p>
                <p className={`${theme.textSecondary} text-xs md:text-sm text-center px-4`}>
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className={`${theme.textSecondary} text-xs`}>
                          {customer.total_invoices || customer.total_orders} orders
                        </p>
                      </div>
                    </div>
